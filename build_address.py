...
db = "feomike"
schema = "census"
theTBL = "tl_2014_44005_addrfeat" #"tl_2014_44005_addrfeat" #"powel" #"working" 
idFld = "tlid"
in_Prj = "4269"
...
def get_az(myID):
	...
	mySQL = mySQL + "POINT( " + str(theSX) + " " + str(theSY) + ")',4269),"
	mySQL = mySQL + "ST_GeomFromText('POINT("
	mySQL = mySQL + str(theEX) + " " + str(theEY) + ")',4269) )); "
...
def mk_pts(num, myID, house, myadd, side, myAz, myGID, myDir):
...
try:
	...
	for row in theCur:
		...
		azimuth = get_az(row[12])
		#for each side, create the desired number of address points
		...
		if row[1] is not None and row[2] is not None:
			...
			if doL == 1:
				mk_pts(lCnt, row[0], row[1], add, "left", azimuth, row[12], ldir)
			if doR == 1:
				mk_pts(rCnt, row[0], row[3], add, "right", azimuth, row[12], rdir)
	updCur.close()
	theCur.close()
	del updCur, theCur
	now = time.localtime(time.time())
